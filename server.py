...
def retrieve(session_id: str, question: str) -> list[str]:
    """
    Multi-angle retrieval — searches with multiple query variations
    to maximise the chance of finding all relevant document sections.
    """
    index, chunks = SESSIONS[session_id]

    # Generate multiple search queries for the same question
    queries = [
    ...
    # Sort by distance (closest first) and return top chunks
...
def ask_claude(question: str, chunks: list[str]) -> dict:
    """Generate a comprehensive answer using Claude Opus with full context."""
    # Use all retrieved chunks for maximum coverage
    context = "\n\n---\n\n".join(chunks)
