...
try:
    ...
    try:
        ...
        with _engine.connect() as _c:
            _rows = _c.execute(_sa_text("PRAGMA table_info(questionnaire_runs)")).fetchall()
            _have = [r[1] for r in _rows]
            _add = [
                ("completed_at",    "ALTER TABLE questionnaire_runs ADD COLUMN completed_at DATETIME"),
                ("industry_sector", "ALTER TABLE questionnaire_runs ADD COLUMN industry_sector VARCHAR(200)"),
                ("processing_secs", "ALTER TABLE questionnaire_runs ADD COLUMN processing_secs FLOAT DEFAULT 0"),
                ("avg_complexity",  "ALTER TABLE questionnaire_runs ADD COLUMN avg_complexity VARCHAR(50)"),
                ("question_count",  "ALTER TABLE questionnaire_runs ADD COLUMN question_count INTEGER DEFAULT 0"),
            ]
            for _n, _s in _add:
                if _n not in _have:
                    try: _c.execute(_sa_text(_s))
...
@app.get("/auth/reset-admin-password", response_class=HTMLResponse)
async def reset_admin_password_get():
    """Visit this URL in browser to reset admin password."""
    seed_email = os.environ.get("ADMIN_EMAIL", "martfiddler@gmail.com")
    seed_pass  = os.environ.get("ADMIN_PASSWORD", "CybeSure2025!")
    try:
        ...
        _db_url = os.environ.get("DATABASE_URL", "sqlite:///./cybesure.db")
        _eng = _CE(_db_url, connect_args={"check_same_thread": False} if "sqlite" in _db_url else {})
        _Sess = _SM(bind=_eng)
        _db = _Sess()
        user = _db.query(User).filter(User.email == seed_email).first()
        if user:
            user.hashed_password = hash_password(seed_pass)
            user.is_active = True
            _db.commit()
            msg = f"Password RESET for {seed_email}"
        else:
            msg = f"User {seed_email} not found — use /auth/bootstrap to create"
        _db.close()
    except Exception as e:
        msg = f"Error: {e}"
    return HTMLResponse(f'''<html><body style="font-family:Arial;background:#0b1829;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:16px">
        <div style="font-size:48px">{"✓" if "RESET" in msg or "not found" not in msg else "✗"}</div>
        <h2 style="color:#00c8e0">{msg}</h2>
        <div style="background:#132030;padding:20px 32px;border:1px solid #00c8e0;text-align:center">
            <div style="color:#7a9cbf;font-size:12px;margin-bottom:8px">LOGIN CREDENTIALS</div>
            <div><strong>Email:</strong> {seed_email}</div>
            <div><strong>Password:</strong> {seed_pass}</div>
        </div>
        <a href="/" style="color:#00c8e0">← Back to login</a>
    </body></html>''')
...
async def upload_questionnaire(
    ...
    # Create a run record to track this questionnaire
    if AUTH_ENABLED and db:
        try:
            token = request.headers.get("Authorization", "").replace("Bearer ", "") if request else ""
            if token:
                user_obj = decode_token(token)
            ...
            print(f"Run record creation error: {e}")
...
async def answer(
    ...
    # If session not found, create an empty one so answering works without documents
    if req.session_id not in SESSIONS:
        empty_session_id = str(uuid.uuid4())
        SESSIONS[empty_session_id] = (None, None, [])
        # Use the new session going forward
        session_id_to_use = empty_session_id
    else:
        session_id_to_use = req.session_id

    _, _, doc_names = SESSIONS[session_id_to_use]
    doc_names_str = ", ".join(doc_names) if doc_names else ""
    session_cache = ANSWER_CACHE.get(session_id_to_use, {})
    ...
    for q in questions_to_process:
        ...
        # 3. Process with AI
        chunks = retrieve(session_id_to_use, q.text)
    ...
    ANSWER_CACHE[session_id_to_use] = session_cache
    ...
    if AUTH_ENABLED and db and org:
        try:
            ...
            now = datetime.utcnow()
            if not run:
                # Create a new run record since none exists
                filename = getattr(req, 'filename', None) or req.session_id or "questionnaire"
                run = QuestionnaireRun(
                    org_id=org.id,
                    user_id=user.id if hasattr(user, 'id') else 0,
                    filename=filename,
                    status="processing",
                    created_at=datetime.utcnow() - timedelta(seconds=max(1, processing_secs))
                )
                db.add(run)
                db.flush()
            ...
            run.processing_secs = round(processing_secs, 1)
...
def send_email(to: str, subject: str, html_body: str) -> bool:
    ...
    print(f"[EMAIL] Attempting to send to={to} subject={subject[:50]}")
    print(f"[EMAIL] SMTP_HOST={SMTP_HOST} SMTP_PORT={SMTP_PORT} SMTP_USER={'SET' if SMTP_USER else 'MISSING'} SMTP_PASS={'SET' if SMTP_PASS else 'MISSING'}")
    if not SMTP_USER or not SMTP_PASS:
        print("[EMAIL] FAILED: SMTP_USER/SMTP_PASS env vars not configured in Render")
        print("[EMAIL] Set SMTP_USER=your-gmail@gmail.com and SMTP_PASS=your-app-password in Render env vars")
        return False
    try:
...
@app.get("/debug/email-test", response_class=HTMLResponse)
async def email_test():
    """Test email configuration — visit in browser to check SMTP settings."""
    smtp_user = os.environ.get("SMTP_USER", "")
    smtp_pass = os.environ.get("SMTP_PASS", "")
    smtp_host = os.environ.get("SMTP_HOST", "smtp.gmail.com")
    ...
    if smtp_user and smtp_pass:
        ...
        success = send_email(
            smtp_user,
            "CybeSure SecureAnswer — Email Test",
            f"""<div style="font-family:Arial;padding:20px">
                <h2 style="color:#00c8e0">✓ Email Configuration Working</h2>
                <p>This test email confirms SMTP is configured correctly.</p>
                <p><strong>SMTP Host:</strong> {smtp_host}:{smtp_port}</p>
                <p><strong>Sending as:</strong> {smtp_user}</p>
                <p><strong>APP_URL:</strong> {app_url}</p>
            </div>"""
        )
        status_html = f"""
        <div style="background:#0a2a1a;border:2px solid #00d4a0;padding:20px;margin-bottom:20px">
            <h2 style="color:#00d4a0;margin:0">{'✓ Test email sent!' if success else '✗ Send failed — check Render logs'}</h2>
            <p style="color:#7a9cbf">Check {smtp_user} inbox</p>
        </div>""" if smtp_user else ""
    else:
        status_html = """
        <div style="background:#2a0a0a;border:2px solid #e84855;padding:20px;margin-bottom:20px">
            <h2 style="color:#e84855;margin:0">✗ SMTP not configured</h2>
            <p style="color:#7a9cbf">Set SMTP_USER and SMTP_PASS in Render environment variables</p>
        </div>"""

    return HTMLResponse(f"""
    <html><body style="font-family:Arial;background:#0b1829;color:#dce8f5;padding:40px;max-width:600px;margin:0 auto">
        <h1 style="color:#00c8e0">Email Configuration Test</h1>
        {status_html}
        <table style="width:100%;border-collapse:collapse">
            <tr><td style="padding:8px;color:#7a9cbf;border-bottom:1px solid #1e3a5f">SMTP_HOST</td>
                <td style="padding:8px;border-bottom:1px solid #1e3a5f">{smtp_host}</td></tr>
            <tr><td style="padding:8px;color:#7a9cbf;border-bottom:1px solid #1e3a5f">SMTP_PORT</td>
                <td style="padding:8px;border-bottom:1px solid #1e3a5f">{smtp_port}</td></tr>
            <tr><td style="padding:8px;color:#7a9cbf;border-bottom:1px solid #1e3a5f">SMTP_USER</td>
                <td style="padding:8px;border-bottom:1px solid #1e3a5f;color:{'#00d4a0' if smtp_user else '#e84855'}">
                    {'✓ ' + smtp_user if smtp_user else '✗ NOT SET'}</td></tr>
            <tr><td style="padding:8px;color:#7a9cbf;border-bottom:1px solid #1e3a5f">SMTP_PASS</td>
                <td style="padding:8px;border-bottom:1px solid #1e3a5f;color:{'#00d4a0' if smtp_pass else '#e84855'}">
                    {'✓ Set (' + str(len(smtp_pass)) + ' chars)' if smtp_pass else '✗ NOT SET'}</td></tr>
            <tr><td style="padding:8px;color:#7a9cbf">APP_URL</td>
                <td style="padding:8px">{app_url}</td></tr>
        </table>
        <div style="margin-top:24px;background:#132030;border:1px solid #1e3a5f;padding:20px">
            <h3 style="color:#f5a623;margin:0 0 12px">How to set up Gmail SMTP</h3>
            <ol style="color:#7a9cbf;font-size:13px;line-height:2">
                <li>Go to <a href="https://myaccount.google.com/security" style="color:#00c8e0">myaccount.google.com/security</a></li>
                <li>Enable 2-Step Verification if not already on</li>
                <li>Search for "App passwords" and create one for "Mail"</li>
                <li>In Render → Environment → add:<br>
                    <code style="color:#00c8e0">SMTP_USER = martfiddler@gmail.com</code><br>
                    <code style="color:#00c8e0">SMTP_PASS = [your 16-char app password]</code>
                </li>
                <li>Redeploy and visit this page again to test</li>
            </ol>
        </div>
        <p style="margin-top:16px"><a href="/" style="color:#00c8e0">← Back to app</a></p>
    </body></html>
    """)
...
class BulkApprovalRequest(BaseModel):
    session_id: str
    questions: List[dict]          # list of {question_id, question, answer, improved_answer}
...
async def review_approval(token: str):
    """Show approval review page — adapts to approver vs reviewer/contributor."""
    ...
    approve_url = f"{APP_URL}/approval/decide?token={token}&decision=approved"
    reject_url  = f"{APP_URL}/approval/decide?token={token}&decision=rejected"
    feedback_url = f"{APP_URL}/approval/feedback?token={token}"

    is_reviewer = rec.get("approver_type") in ("reviewer", "contributor")
    status_colour = {"approved": "#00d4a0", "rejected": "#e84855", "pending": "#f5a623"}.get(rec["status"], "#f5a623")
    role_label = "Feedback" if is_reviewer else "Approval Review"
    page_title = "Share Your Feedback" if is_reviewer else "Answer Approval Review"

    if rec["status"] != "pending":
        action_html = f"""
        <div class="decided">
          <p style="font-size:18px;font-weight:600;color:{status_colour}">{rec['status'].upper()}</p>
          <p style="color:#7a9cbf">{rec.get('comments') or 'No comments'}</p>
        </div>"""
    elif is_reviewer:
        # Reviewer/contributor: feedback only, no approve/reject
        action_html = f"""
        <div class="card">
          <div class="label">Your Feedback</div>
          <p style="color:#7a9cbf;font-size:13px;margin:0 0 16px">
            Please add any comments, context, or suggested improvements below.
            Your feedback will be sent to the requester.
          </p>
          <textarea class="comments" id="comments" placeholder="Add your feedback, context, or suggested improvements here..."></textarea>
          <div style="display:flex;gap:12px;align-items:center">
            <button onclick="submitFeedback()" style="background:#00c8e0;border:none;color:#0f2035;padding:12px 32px;font-weight:700;font-size:14px;border-radius:3px;cursor:pointer;font-family:'Montserrat',sans-serif">
              ✉ Send Feedback
            </button>
            <span id="feedbackMsg" style="font-size:13px;color:#00d4a0;display:none">✓ Feedback sent — thank you</span>
          </div>
        </div>
        <script>
          function submitFeedback() {{
            const c = document.getElementById('comments').value.trim();
            if(!c) {{ alert('Please add some feedback before sending.'); return; }}
            fetch('{feedback_url}', {{
              method: 'POST',
              headers: {{'Content-Type': 'application/json'}},
    ...
    <html>
    <head>
      <title>CybeSure — {page_title}</title>
      ...
      <style>
        ...
        .comments {{width:100%;background:#152a42;border:1px solid #1e3a5a;color:#dce8f5;padding:12px;font-size:14px;border-radius:3px;margin-bottom:16px;min-height:100px;font-family:'Open Sans',sans-serif;box-sizing:border-box}}
        .decided {{background:#1a2f4a;border:1px solid #2d5a80;padding:20px;border-radius:4px;text-align:center}}
        .role-tag {{display:inline-block;background:#1e3a5f;color:#7a9cbf;font-size:11px;font-weight:700;padding:3px 10px;border-radius:2px;margin-bottom:20px;text-transform:uppercase;letter-spacing:0.5px}}
      </style>
    </head>
    <body>
    <div class="header">
      <div class="logo">Cybe<em>Sure</em></div>
      <span style="color:#7a9cbf;font-size:14px;margin-left:8px">{page_title}</span>
    </div>
    <div class="container">
      <div class="card">
        <div class="role-tag">{'Contributor / Reviewer' if is_reviewer else 'Approver'}</div>
...
@app.post("/approval/bulk-request")
async def bulk_request_approval(req: BulkApprovalRequest, background_tasks: BackgroundTasks):
    """Send all answers in one email to approver and/or contributor."""
    if not req.approver_email and not req.contributor_email:
        raise HTTPException(400, "At least one recipient (approver or contributor) is required")
    if not req.questions:
        raise HTTPException(400, "No questions provided")

    def build_answers_html(questions: list, include_approve: bool) -> str:
        rows = ""
        for idx, q in enumerate(questions, 1):
            approve_token = hashlib.sha256(f"{req.session_id}-{q['question_id']}-{uuid.uuid4()}".encode()).hexdigest()[:32]
            APPROVALS[approve_token] = {
                "token": approve_token,
                "session_id": req.session_id,
                ...
                "approver_email": req.approver_email or req.contributor_email,
                "approver_name": req.approver_name or req.contributor_name,
                "approver_type": "approver" if include_approve else "reviewer",
                "reviewer_email": None,
                "reviewer_name": None,
                "requester_name": req.requester_name,
                "status": "pending",
            ...
            review_url = f"{APP_URL}/approval/review/{approve_token}"
            approve_url = f"{APP_URL}/approval/decide?token={approve_token}&decision=approved"
            reject_url  = f"{APP_URL}/approval/decide?token={approve_token}&decision=rejected"

            action_html = f"""
            <div style="margin-top:12px;display:flex;gap:10px">
              <a href="{approve_url}" style="display:inline-block;background:#00c8e0;color:#0f2035;padding:8px 20px;text-decoration:none;font-weight:700;font-size:13px;border-radius:3px">✓ Approve</a>
              <a href="{reject_url}" style="display:inline-block;background:transparent;color:#e84855;padding:8px 20px;text-decoration:none;font-weight:700;font-size:13px;border-radius:3px;border:2px solid #e84855">✗ Reject</a>
            </div>""" if include_approve else f"""
            <div style="margin-top:12px">
              <a href="{review_url}" style="display:inline-block;background:#00c8e0;color:#0f2035;padding:8px 20px;text-decoration:none;font-weight:700;font-size:13px;border-radius:3px">✉ Add Feedback</a>
            </div>"""

            rows += f"""
            <div style="background:#fff;border:1px solid #e2e8f0;border-radius:4px;padding:20px;margin-bottom:16px">
              <p style="font-size:11px;color:#7a9cbf;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin:0 0 6px">Q{idx}</p>
              <p style="color:#0f2035;font-size:15px;font-weight:600;margin:0 0 14px;line-height:1.5">{q['question']}</p>
              <p style="font-size:11px;color:#7a9cbf;font-weight:700;text-transform:uppercase;margin:0 0 6px">Proposed Answer</p>
              <p style="color:#334155;font-size:14px;line-height:1.7;padding:12px;background:#f8fafc;border-left:3px solid #00c8e0;margin:0 0 10px">{q['answer']}</p>
              {f"<p style='font-size:11px;color:#7a9cbf;font-weight:700;text-transform:uppercase;margin:0 0 6px'>Enhanced Answer</p><p style='color:#334155;font-size:14px;line-height:1.7;padding:12px;background:#f0fdf4;border-left:3px solid #00d4a0;margin:0'>{q.get('improved_answer','')}</p>" if q.get('improved_answer') else ""}
              {action_html}
            </div>"""
        return rows
    ...
    # Send to approver
    if req.approver_email and req.approver_name:
        rows_html = build_answers_html(req.questions, include_approve=True)
        note_html = f"<div style='background:#fff3cd;border:1px solid #ffc107;padding:12px;border-radius:3px;margin-bottom:20px'><p style='color:#856404;margin:0'><strong>Note from {req.requester_name}:</strong> {req.note}</p></div>" if req.note else ""
        body = f"""
        <div style="font-family:Arial,sans-serif;max-width:720px;margin:0 auto">
          ...
          <div style="padding:32px;background:#f9fafb;border:1px solid #e2e8f0">
            <h2 style="color:#0f2035;font-size:18px;margin:0 0 8px">Questionnaire Approval Required</h2>
            <p style="color:#4a6a8a;font-size:14px;margin:0 0 20px">
              Hi <strong>{req.approver_name}</strong>, <strong>{req.requester_name}</strong> is requesting your approval on
              <strong>{len(req.questions)} questionnaire answer{'s' if len(req.questions) != 1 else ''}</strong>.
            </p>
            {note_html}
            {rows_html}
          </div>
        ...
        background_tasks.add_task(send_email, req.approver_email,
            f"Approval Required: {len(req.questions)} answers — CybeSure SecureAnswer", body)
        results.append(f"Sent to approver: {req.approver_name}")

    # Send to contributor
    if req.contributor_email and req.contributor_name:
        rows_html = build_answers_html(req.questions, include_approve=False)
        note_html = f"<div style='background:#fff3cd;border:1px solid #ffc107;padding:12px;border-radius:3px;margin-bottom:20px'><p style='color:#856404;margin:0'><strong>Note from {req.requester_name}:</strong> {req.note}</p></div>" if req.note else ""
        body = f"""
        <div style="font-family:Arial,sans-serif;max-width:720px;margin:0 auto">
          <div style="background:#0f2035;padding:24px 32px;border-bottom:3px solid #00c8e0">
          ...
          <div style="padding:32px;background:#f9fafb;border:1px solid #e2e8f0">
            <h2 style="color:#0f2035;font-size:18px;margin:0 0 8px">Your Feedback Requested</h2>
            <p style="color:#4a6a8a;font-size:14px;margin:0 0 20px">
              Hi <strong>{req.contributor_name}</strong>, <strong>{req.requester_name}</strong> is requesting your feedback on
              <strong>{len(req.questions)} questionnaire answer{'s' if len(req.questions) != 1 else ''}</strong>.
              Please click the feedback button on any answer you wish to comment on.
            </p>
            {note_html}
            {rows_html}
          </div>
        ...
        background_tasks.add_task(send_email, req.contributor_email,
            f"Feedback Requested: {len(req.questions)} answers — CybeSure SecureAnswer", body)
        results.append(f"Sent to contributor: {req.contributor_name}")

    return {"status": "sent", "message": " | ".join(results), "questions_included": len(req.questions)}


@app.post("/approval/feedback")
async def submit_feedback(token: str, req: FeedbackRequest, background_tasks: BackgroundTasks):
    """Receive feedback from a reviewer or contributor."""
    if token not in APPROVALS:
    ...
    <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto">
      ...
      <div style="padding:32px;background:#f9fafb;border:1px solid #e2e8f0">
        <h2 style="color:#00c8e0;font-size:18px;margin:0 0 8px">Feedback Received</h2>
        <p style="color:#4a6a8a;font-size:14px;margin:0 0 20px">
          <strong>{rec['approver_name']}</strong> has provided feedback on the answer to:
        </p>
        <p style="color:#0f2035;font-weight:600;font-size:15px;margin:0 0 16px">{rec['question']}</p>
        <div style="background:#fff3cd;border:1px solid #ffc107;padding:12px 16px;border-radius:3px;margin-bottom:16px">
          <p style="font-weight:700;color:#856404;margin:0 0 4px">Feedback:</p>
          <p style="color:#856404;margin:0">{comments}</p>
        </div>
        <p style="color:#4a6a8a;font-size:13px">Log in to CybeSure SecureAnswer to review this feedback.</p>
      </div>
    </div>"""
    background_tasks.add_task(send_email, SMTP_USER, f"Feedback received: {rec['question'][:60]}...", notify_html)
...
async def decide_approval(token: str, decision: str, comments: str = "", background_tasks: BackgroundTasks = None):
    ...
    <body>
      <div class="box">
        ...
        <h2>{status_word}</h2>
        <p>Your decision has been recorded and the requester has been notified.</p>
        {f"<p style='background:#1e3a5f;padding:12px;border-radius:3px;color:#dce8f5;margin-top:16px'><strong>Your comments:</strong> {comments}</p>" if comments else ""}
        <button onclick="window.close()" style="margin-top:24px;background:#00c8e0;border:none;color:#0b1829;padding:12px 32px;font-family:Montserrat,sans-serif;font-size:14px;font-weight:700;cursor:pointer;border-radius:2px">
          Close Window
        </button>
        <p style="margin-top:12px;font-size:12px">If the button doesn't close this tab, you can close it manually.</p>
        <div class="status">{"✅" if decision == "approved" else "❌"}</div>
        <h2>{status_word}</h2>
