...
def fetch_all_from_url(url: str) -> list[tuple[str, bytes]]:
    ...
    if 'drive.google.com/file' in url or 'docs.google.com' in url:
        ...
        if file_id:
            try:
                fname, data = fetch_document_from_url(f"https://drive.google.com/uc?export=download&id={file_id}")
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    for i in range(0, len(texts), EMBED_BATCH):
        resp = openai_client.embeddings.create(input=texts[i:i+EMBED_BATCH], model=EMBEDDING_MODEL)
...
async def upload_documents_url(req: UrlRequest):
    ...
    for url in req.urls:
        try:
            ...
            files_failed.append(url)
    if not raw:
        raise HTTPException(400, "Could not fetch or extract text from the provided URLs. Ensure files are publicly accessible.")
    result = build_index(raw, session_id)
