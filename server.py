...
async def answer(
    ...
    for q in req.questions:
        ...
        # 2. Check persistent question cache (same question answered before)
...
async def export(req: ExportRequest):
    ...
    if fmt == "excel":
        ...
        # ── FORMAT 1: Original file with answers filled in (preserve all columns)
        if req.original_file:
            try:
                ...
                for row in range(2, ws.max_row + 1):
                    ...
                    sc.value = r.document_names or " | ".join(r.sources)
                ...
                ws.column_dimensions[get_column_letter(conf_col)].width = 14
                ws.column_dimensions[get_column_letter(pct_col)].width  = 14
                ws.column_dimensions[get_column_letter(src_col)].width  = 40
                ws.column_dimensions[get_column_letter(appr_col)].width = 22

                buf1 = io.BytesIO()
                wb.save(buf1)
                buf1.seek(0)

                # Return the filled original file
        ...
        for r in results:
            row_data = [
                ...
                r.confidence,
                f"{r.confidence_pct}%",
                r.document_names or " | ".join(r.sources),
                get_approver(r)
            ]
            ri = r.question_id + 2
