...
async def debug_status(db=Depends(get_db)):
    """Shows DB status and config — remove after debugging."""
    status = {
        "auth_enabled": AUTH_ENABLED,
        "admin_email_env": os.environ.get("ADMIN_EMAIL", "NOT SET — using martfiddler@gmail.com"),
    ...
    if AUTH_ENABLED and db:
        try:
            ...
            status["org_count"] = org_count
            if user_count > 0:
                users = db.query(User).all()
                status["users"] = [{"email": u.email, "role": u.role, "active": u.is_active} for u in users]
        except Exception as e:
            status["db_error"] = str(e)
    return status
async def reset_admin_password_get(db=Depends(get_db)):
    """GET version — just visit this URL in browser to reset."""
    return await reset_admin_password(db)
...
async def reset_admin_password(db=Depends(get_db)):
    ...
    Sets password to ADMIN_PASSWORD env var (or CybeSure2025!).
    Only works if ADMIN_EMAIL env var matches an existing account.
    """
    ...
    if not user:
        # Create if missing
        return await bootstrap(db)
    user.hashed_password = hash_password(seed_pass)
    user.is_active = True
    db.commit()
    return {"message": f"Password reset for {seed_email}", "password": seed_pass}
async def bootstrap(db=Depends(get_db)):
    """
    Create the first admin account when DB is empty.
    Only works if no users exist — safe to leave enabled.
    Uses ADMIN_EMAIL / ADMIN_PASSWORD env vars or defaults.
    """
    if not AUTH_ENABLED or db is None:
        raise HTTPException(503, "Auth not available")
    if db.query(User).count() > 0:
        raise HTTPException(400, "Bootstrap not available — users already exist. Use /auth/register.")
    ...
    slug = seed_org.lower().replace(" ", "-")
    org = Organisation(
        name=seed_org, slug=slug, contact_email=seed_email,
        tier="test", status="active", questionnaire_limit=999,
        subscription_end=datetime.utcnow() + timedelta(days=3650)
    )
    db.add(org); db.flush()
    admin = User(
        org_id=org.id, email=seed_email, full_name="Admin",
        hashed_password=hash_password(seed_pass),
        role="admin", is_active=True
    )
    db.add(admin); db.commit()
    return {
        "message": f"Admin account created: {seed_email}",
        "email": seed_email,
        "tier": "test",
