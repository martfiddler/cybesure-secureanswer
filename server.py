    ...
    try:
        with _engine.connect() as conn:
            existing = [row[1] for row in conn.execute(
                __import__('sqlalchemy').text("PRAGMA table_info(questionnaire_runs)")
            )]
            new_cols = {
                ...
                'avg_complexity':  'ALTER TABLE questionnaire_runs ADD COLUMN avg_complexity VARCHAR(50) DEFAULT "average"',
