...
@app.get("/auth/reset-admin-password")
async def reset_admin_password_get(db=Depends(get_db)):
    """GET version — visit this URL in browser to reset password."""
    ...
    if not user:
        ...
        return {"message": f"Admin account CREATED: {seed_email}", "password": seed_pass}
